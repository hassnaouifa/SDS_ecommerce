...
export default function EditFormationModal({ formation, onClose, onSave }) {
  const [formData, setFormData] = useState(formation);

  // 1. Fonction pour transformer le HTML entrant en Texte Propre (pour l'affichage)
  const formatHtmlToText = (html) => {
    if (!html) return "";
    let text = html;
    // Ajoute un saut de ligne après les paragraphes, titres et éléments de liste
    text = text.replace(/<\/(p|h\d|div|li)>/ig, '\n'); 
    // Remplace le début des balises <li> par un tiret pour faire joli
    text = text.replace(/<li>/ig, '- '); 
    // Supprime toutes les autres balises HTML restantes (<p>, <ul>, etc.)
    text = text.replace(/<[^>]*>?/gm, ''); 
    // Nettoie les espaces en trop
    return text.trim();
  };

  // 2. Fonction pour transformer le Texte en HTML (pour l'enregistrement vers Odoo)
  const formatTextToHtml = (text) => {
    if (!text) return "";
    // Odoo va ajouter un <p> global (grâce à ton code Python), 
    // donc ici on convertit juste les sauts de ligne "Entrée" en vrais sauts de ligne HTML <br>
    return text.replace(/\n/g, '<br />');
  };
  ...
  useEffect(() => {
    if (formation) {
      setFormData({
        ...formation,
        // On nettoie la description dès l'ouverture de la modale
        description_ecommerce: formatHtmlToText(formation.description_ecommerce)
      });
    }
  }, [formation]);
  ...
  const handleSubmit = (e) => {
    ...
    const dataToSave = {
      ...formData,
      // On re-transforme le texte brut en format compatible HTML avant d'envoyer à Odoo
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#10174f]/30 backdrop-blur-sm">
      <div className="bg-white w-full max-w-3xl rounded-[20px] shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#ececf5] bg-[#f8f9fc]">
          <h2 className="text-lg font-bold text-[#10174f]">Modifier la formation</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1">
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="w-full md:w-2/3 space-y-5">
              ...
              <div>
                {/* L'étiquette a changé puisqu'il n'y a plus de code HTML visible ! */}
          ...
          <div className="flex items-center justify-end gap-3 pt-6 mt-6 border-t border-[#ececf5]">
            ...
            >
              <Save size={18} strokeWidth={2.5} /> Enregistrer
            </button>
          </div>
        </form>
      </div>
